...
| 경로 | 내용 |
|---|---|
| `app/`, `public/` … | CodeIgniter 4 프로젝트 본체 |
| `docs/epNN.md` | 각 회차의 원고 원본 |
| `assets/cover/` | 대표 이미지와 생성 스크립트 |
| `CURRICULUM.md` | 강좌 기획서 — ADR, 전체 DB 스키마, 37편 목차 |
| `_design/` | 설계 검증본. 회차별 코드가 아니다 ([설명](_design/README.md)) |

`_design/` 은 강좌를 쓰기 전에 스키마가 맞는지 확인하려고 미리 만들어 둔 전체 설계본입니다.
회차별 코드는 `epNN` 태그를 보세요.
