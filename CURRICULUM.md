...
| 에디터 | 초반 `<textarea>` → **25편**에서 교체 | 의존성을 늦게 도입. 이미지 삽입이 24~25편 업로드 라우트를 그대로 쓴다 |
...
> **25편의 에디터 범위**: 외부 라이브러리를 **붙이는 것까지**만 한다. `boards.use_editor` 가 1이면 본문 칸을 에디터로 바꾸고, 에디터의 이미지 업로드 요청을 24편에서 만든 업로드 라우트로 보낸다. 라이브러리 선택은 25편 착수 시점에 확정하되 기준은 둘이다 — **CDN 한 줄로 붙을 것**, **빌드 도구를 요구하지 않을 것**(ADR-002의 "최소 JS"). 툴바 커스터마이징, 에디터 안에서의 이미지 리사이즈, 마크다운 전환은 범위 밖이다.
...
| 25 | 파일 첨부 ②: 썸네일 · 안전한 다운로드 · 에디터 | CI4 Image, 권한 체크 다운로드 라우트, `use_editor` 토글로 `<textarea>` 교체 | 24 |
...
| 25편이 세 주제(썸네일·다운로드·에디터)로 무거워짐 | 에디터는 붙이는 것까지만. 파트 5 표 아래 범위 선언을 지킨다 |
